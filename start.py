...
class song:
    ...
    def update_song_info(self):
        ...
        if len(str(self.name)) > 70:
            song_name_label['text'] = str(self.name)[:70] + "...."
        else:
        ...
        if len(self.path) > 70:
            song_path_label['text'] = self.path[:70] + "...."
        else:
...
def on_progressbar_click(event):
    ...
    progressbar_width = event.widget.winfo_width()

    # Get the click position relative to the progress bar
    click_x = event.x

    # Calculate the click position as a percentage of the total width
    click_position_percent = (click_x / progressbar_width) * 100
    click_position_percent = (click_position_percent / 100) * new_song.length
    pygame.mixer.music.pause()
    pygame.mixer.music.set_pos(click_position_percent)
    pygame.mixer.music.unpause()
...
# progress.bind("<Button-1>", on_progressbar_click)
# ===============================================================
